...
Live case 没有运行 confirmation：最佳候选的 3.77% 提升低于 5% 阈值。
...
- pytest（本分支通过 `pytest --collect-only` 收集 **623** 个测试；CI 运行同一测试集）
...
| 项目 | 当前状态 |
|---|---|
| Unit / CPU tests | **623** collected（本分支运行 `pytest --collect-only`） |
| Golden workloads | 5 |
