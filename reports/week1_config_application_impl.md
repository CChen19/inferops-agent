...
# Must match inferops.observability.init_mlflow default (or override both sides together):
export MLFLOW_TRACKING_URI="${MLFLOW_TRACKING_URI:-sqlite:///mlruns.db}"
...
# Terminal 2 — capture OLD occupant PID automatically, then run with live PID probe
export MLFLOW_TRACKING_URI="${MLFLOW_TRACKING_URI:-sqlite:///mlruns.db}"
export OLD_PID="$(ss -ltnp 'sport = :8000' | sed -n 's/.*pid=\([0-9]\+\).*/\1/p' | head -n1)"
...
def on_progress(msg: str) -> None:
    ...
    if m:
        ...
        assert child_i != old_pid, (child_i, old_pid)  # must be a NEW identity
...
print("status=", result.status.value, "actual keys=", sorted(result.actual_config or {}))
...
data = __import__("json").loads(probe.read_text())
assert data["child_pid"] != old_pid, (data["child_pid"], old_pid)
...
assert probe["child_pid"] != old_pid, (probe["child_pid"], old_pid)
...
print("OK live listener==child==sqlite process_pid", probe["child_pid"], "!= OLD_PID", old_pid)
PY
...
- [ ] Live child PID ≠ pre-restart `OLD_PID` (**scripted assert** in the copy-paste above)
...
### MLflow tags (same tracking URI as the bench + resolve by **name**)

`init_mlflow()` sets the URI **inside the bench process only**. A later standalone
Python must set the **same** URI explicitly (env or `set_tracking_uri`). Default
in code: `sqlite:///mlruns.db`. Experiment name: `inferops`.

```bash
# Use the SAME URI the bench used (export before both the bench and this probe):
export MLFLOW_TRACKING_URI="${MLFLOW_TRACKING_URI:-sqlite:///mlruns.db}"
...
# Critical: match inferops.observability.init_mlflow / _MLFLOW_TRACKING_URI
...
if exp is None:
    raise SystemExit(
        f"MLflow experiment {name!r} not found at {uri!r}. "
        f"List: {[(e.experiment_id, e.name) for e in client.search_experiments()]}"
    )
...
assert runs, f"no runs found in experiment {name!r} at {uri!r}"
for r in runs:
    print(
        r.info.run_id,
...
**Startup inject timing:** `INFEROPS_SIMULATE_STARTUP_FAILURE=oom|timeout|identity`
is evaluated **after spawn / before `wait_ready_verbose`**, so the path is fast and
does **not** wait on real `/health`.
...
### B2. Startup failure — simulated OOM (before readiness wait)
...
### B3. Startup failure — simulated timeout (before readiness wait)

```bash
export MLFLOW_TRACKING_URI="${MLFLOW_TRACKING_URI:-sqlite:///mlruns.db}"
unset INFEROPS_SIMULATE_STOP_FAILURE
export INFEROPS_SIMULATE_STARTUP_FAILURE=timeout
python - <<'PY'
from inferops.tools.run_benchmark import RunBenchmarkInput, run_benchmark
from inferops.bench_runner import StartupTimeoutError
try:
    run_benchmark(RunBenchmarkInput(
        experiment_id="w1_cfg_timeout_fail",
        workload_name="chat_short",
        persist=True,
        session_id="w1cfg_",
    ))
    ...
    assert exc.result is not None and exc.result.status.value == "failed"
    print("OK timeout-failure", exc.result.run_id)
else:
    raise SystemExit("expected StartupTimeoutError")
PY
unset INFEROPS_SIMULATE_STARTUP_FAILURE
```

### B4. Startup failure — simulated identity failure (before readiness wait)
...
- [ ] oom/timeout/identity complete without waiting on real `/health`
...
Targeted cases: healthy-but-different restart, external health-only, identity change, stop failure, listener≠child, stale unknown PID, mismatch→invalid, OOM cleanup + run_id, simulate-stop / simulate-startup (`oom`/`timeout`/`identity`) **before** readiness wait, live identity probe file, `run_benchmark` persists failed row.
